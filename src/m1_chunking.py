...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    curr_parent = ""
    for p in paragraphs:
        if curr_parent and (len(curr_parent) + len(p) + 2 > parent_size):
            parent_groups.append(curr_parent.strip())
            curr_parent = ""
        curr_parent += (p + "\n\n")
        ...
        if re.match(r'^#{1,3}\s+', part.strip()):
            if current_header or current_content:
                ...
                if chunk_text:
                    chunks.append(Chunk(
                        text=chunk_text,
                        metadata={**metadata, "section": current_header or "Introduction", "strategy": "structure", "chunk_index": len(chunks)}
                    ))
